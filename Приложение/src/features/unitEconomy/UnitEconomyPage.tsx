...
export function UnitEconomyPage({ accountId }: Props) {
  ...
  const [activeTable, setActiveTable] = useState<'china' | 'unit' | 'cost' | 'logistics' | 'ads' | 'margin' | 'summary'>('summary')
    ...
    <div className="grid ue-grid">
      <div className="card ue-card">
        ...
        <div className="ue-tabs">
          <button className={activeTable === 'china' ? 'btn primary' : 'btn'} onClick={() => setActiveTable('china')}>Китай</button>
          <button className={activeTable === 'unit' ? 'btn primary' : 'btn'} onClick={() => setActiveTable('unit')}>Unit экономика</button>
